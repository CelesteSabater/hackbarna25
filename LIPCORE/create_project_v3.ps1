...
<button type="button" id="send">Genera vídeo</button>
</div>
</form>
...
document.getElementById('send').addEventListener('click', async () => {
    ...
    if (!audioInput.files.length) { alert('Puja un fitxer d\\'àudio'); return; }
...
def generate():
    ...
    # si no té extensió coneguda, acceptem igualment, però recomanem usar mp3/wav
...
==================================
Aquest projecte està configurat per pujar UNA IMATGE i UN FITXER D'AUDIO. No cal escriure text.
...
6) Obre la PUBLIC_URL al navegador i fes la demo: puja una foto i un fitxer d'àudio, prem "Genera vídeo".
Notes:
- Cada generació consumeix crèdit a fal.ai. Prova amb fitxers curts i resolució 480p.
- No comparteixis la teva FAL_KEY públicament.
- Els formats d'àudio recomanats: mp3 o wav.
